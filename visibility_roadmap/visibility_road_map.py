...
class VisibilityRoadMap:
    ...
    def planning(self, start_x, start_y, goal_x, goal_y, obstacles):
        ...
        self.nodes = self.generate_graph_node(start_x, start_y, goal_x, goal_y,
                                         obstacles)

        self.road_map_info = self.generate_road_map_info(self.nodes, obstacles)

        if self.do_plot:
            self.plot_road_map(self.nodes, self.road_map_info)
        #     plt.pause(1.0)
