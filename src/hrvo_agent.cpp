...
void Robot::set_pid_controller(double kpRho, double kpAlpha, double kpBeta, double dt) {
    ...
    double dx = curr_goal_.getX() - curr_pos_.getX();
    double dy = curr_goal_.getY() - curr_pos_.getY();
    double theta = atan2(dy, dx) - M_PI;
    this->control_dt_ = dt * 1000; // milli sec

    vector<double> state{curr_pos_.getX(), curr_pos_.getY(), theta};
    cntrl_ = make_unique<controller>(state, kpRho, kpAlpha, kpBeta, dt, radius_);
...
#ifndef HRVO_SIMULATION_DRIVE
        this_thread::sleep_for(sim_time_);
#endif
